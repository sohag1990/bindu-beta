...
func ModelGenerator(cmd *cobra.Command, args []string) {
	plural := pluralize.NewClient()
	//Flag value
	hasOne := cmd.Flag("hasOne")
	hasMany := cmd.Flag("hasMany")
	manyToMany := cmd.Flag("manyToMany")
	// hasOneThrough := cmd.Flag("hasOneThrough")
	// hasManyThrough := cmd.Flag("hasManyThrough")
	belongsTo := cmd.Flag("belongsTo")
	hasOneValue := fmt.Sprintf("%v", hasOne.Value)
	hasManyValue := fmt.Sprintf("%v", hasMany.Value)
	manyToManyValue := fmt.Sprintf("%v", manyToMany.Value)
	// hasOneThroughValue := fmt.Sprintf("%v","hasOneThrough.Value")
	// hasManyThroughValue := fmt.Sprintf("%v","hasManyThrough.Value")
	belongsToValue := fmt.Sprintf("%v", belongsTo.Value)
	// fmt.Println(hasManyValue, hasOneValue, belongsToValue)
	// return
	// fmt.Println(args)
	...
	if len(hasOneValue) > 0 {
		splitHasOneData := strings.Split(hasOneValue, "^")
		firstModel := strings.Split(splitHasOneData[0], ":")
		f.WriteString("\t" + firstModel[0] + " " + firstModel[0] + "\n")
		modelIfNotExistCreate(firstModel[0], args)
	}
	if len(belongsToValue) > 0 {
		belongsToModel := strings.Split(belongsToValue, ":")
		f.WriteString("\t" + belongsToModel[0] + " " + belongsToModel[0] + "\n")
		f.WriteString("\t" + belongsToModel[0] + "ID   uint64\n")
		modelIfNotExistCreate(belongsToModel[0], args)
	}
	if len(hasManyValue) > 0 {
		splitHasManyData := strings.Split(hasManyValue, "^")
		firstModel := strings.Split(splitHasManyData[0], ":")
		f.WriteString("\t" + firstModel[0] + " []" + firstModel[0] + "\n")
		modelIfNotExistCreate(firstModel[0], args)
	}
	if len(manyToManyValue) > 0 {
		splitManyToManyData := strings.Split(manyToManyValue, "^")
		firstModel := strings.Split(splitManyToManyData[0], ":")
		f.WriteString("\t" + firstModel[0] + " []" + firstModel[0] + " `gorm:\"many2many:" + args[1] + "_" + plural.Plural(firstModel[0]) + ";\"`\n")
		modelIfNotExistCreateManyToMany(firstModel[0], args)
	}
	f.WriteString("}")
	defer f.Close()
